...
jQuery(document).ready(function handleDocumentReady (e) {
  jQuery("#close").bind("click", function() {
	...
	compoundGenusAll(allGenusCB,"#loadingIcon");
	compoundSpeciesAll(allSpeciesCB,"#loadingIcon");
...
jQuery(document).ready(function handleDocumentReady (e) {
  ...
  MarvinJSUtil.getEditor("#sketch").then(function (sketcherInstance) {
	...
	if(window.location.hash.substring(1) == 'results'){
		var storedBasicSearchResultArr = JSON.parse(sessionStorage.getItem("BSResults"));
...
function populateForm(url_params_object){
	...
	if(url_params_object['smiles']){
		var marvinSmiles= marvinSketcherInstance.importStructure("smiles",url_params_object['smiles']);
		...
		if(jQuery('#customFormulaElement'+counter).val()){
			if(jQuery('#customFormulaOperator'+counter).val())
				if(jQuery('#customFormulaValue'+counter).val()){
					obj['formulaElemName'] = jQuery('#customFormulaElement'+counter).val();
					obj['formulaElemOp'] = jQuery('#customFormulaOperator'+counter).val();
...
function webservices_params(form_data, smiles, offset, limit){
  ...
  // get value in the form
	...
	if (typeof form_data.origintype === 'undefined')
		var otype="";
	
	else 
		var otype=form_data.origintype;

	if((name!="" || minMolWt!="" || maxMolWt!="" || formula!="" || genus!="" || smilesForm!= "" || species!="" || inchi!="" || inchikey!="" || mass!="" || (otype!="" && otype!="all"&& otype!="Select an origin type") )|| smiles.length!=0 || customFormulaElement )// something in the form or in marvin
	{
		...
		if(search_type =='SIMILARITY'){
			...
			var search_options={"queryStructure": smiles,"searchType": "SUBSTRUCTURE"};
		...
		if(name!="" || minMolWt!="" || maxMolWt!="" || formula!="" || genus!="" || customFormulaElement || smilesForm!= "" || species!="" || inchi!="" || inchikey!="" || mass!="" || (otype!="" && otype!="all"&& otype!="Select an origin type")) // the form is completed
		 {
		   //console.log('SearchOptions :', search_options);
				...
				if(minMolWt)
				   conditions.$and.push({"compound_molecular_weight":{"$gte":parseFloat(minMolWt)}});
				...
				for(var counter=0; counter <= (maxLoopComp); counter++){
					if(document.getElementById('customFormulaElement' + counter)){
						elementName = document.getElementById('customFormulaElement' + counter).value;
						if(elementName){
							if(document.getElementById('customFormulaOperator' + counter)){
								operatorValue = document.getElementById('customFormulaOperator' + counter).value;
								if(operatorValue){
									if(document.getElementById('customFormulaValue' + counter)){
										value = document.getElementById('customFormulaValue' + counter).value;
										if(value){
											...
											if(operatorValue == 'lessThan')
												...
												tempOperator = "$gt";
											else if(operatorValue == 'equalTo')
												tempOperator = "$eq";
											
											var tempString2 = '{"' + tempString + '":{"' + tempOperator + '":' + value + '}}'; 
				...
				if(maxMolWt)
				   conditions.$and.push({"compound_molecular_weight":{"$lte":parseFloat(maxMolWt)}});
				if(formula!="")
					conditions.$and.push({"compound_molecular_formula":{"$eqIc":[formula]}});
				if(genus!="")
					conditions.$and.push({"genus":{"$eqIc":[genus]}});
				if(species!="")
				...
				//	else if (mass_select == "eq"){
						if (mass_precision){
							...
							if(mass_precision_unit =="Dalton")
								...
								var precision=parseFloat(mass)*parseFloat(mass_precision)/parseFloat(1e6);
							
							console.log('precision: ',precision);
							conditions.$and.push({"compound_accurate_mass":{"$gte":parseFloat(mass)-precision}});
							conditions.$and.push({"compound_accurate_mass":{"$lte":parseFloat(mass)+precision}});
				...
				params = buildParamsWebServiceCall(search_options, offset, limit, conditions, error);
				
		}
		 else // nothing in the formula
		 {
		   //console.log('SearchOptions :', search_options);
		   var conditions = "";
		   params = buildParamsWebServiceCall(search_options, offset, limit, conditions, error);
...
function create_result_table(results,length,offset){
	...
	for(i=0;i<Math.min(parseInt(length),nb_results);i++){
		...
		number_div.innerHTML="<h1 class='g-title'>#"+(parseInt(i)+1+parseInt(offset)).toString()+"<h1>";	
	//	number_div.style.textAlign = "center";
		...
		var npa_id = 'NPA'+ results.data[i].compound_id.toString().padStart(6,"0");
	...
	//	a.setAttribute("target","_blank");
		...
		line.appendChild(th);	var td=document.createElement("td");		
		var a= document.createElement('a');    a.innerHTML=results.data[i].compound_cluster_id;    a.title =results.data[i].compound_cluster_id;	   
		...
		var a= document.createElement('a');    a.innerHTML=results.data[i].compound_node_id;    a.title =results.data[i].compound_node_id;
		...
		var td=document.createElement("td");	td.innerHTML=results.data[i].compound_names;	line.appendChild(td);	table.appendChild(line);
		...
		line.appendChild(th);	var td=document.createElement("td");	td.innerHTML=results.data[i].compound_molecular_formula;	line.appendChild(td);	table.appendChild(line);
		...
		line.appendChild(th);	var td=document.createElement("td");	td.innerHTML=results.data[i].compound_molecular_weight.toFixed(2);	line.appendChild(td);	table.appendChild(line);
		...
		line.appendChild(th);	var td=document.createElement("td");	td.innerHTML=results.data[i].compound_accurate_mass.toFixed(4);	line.appendChild(td);	table.appendChild(line);
		...
		line.appendChild(th);	var td=document.createElement("td");	td.innerHTML=results.data[i].origin_type;	line.appendChild(td);	table.appendChild(line);
		...
		var td=document.createElement("td");	td.innerHTML=results.data[i].genus;	line.appendChild(td);	table.appendChild(line);	var line=document.createElement("tr");
		...
		td.innerHTML=results.data[i].origin_species; line.appendChild(td);	table.appendChild(line);	var line=document.createElement("tr");	var th=document.createElement("th");
		th.style.textTransform= "uppercase";	th.innerHTML="InchIKey";	line.appendChild(th);	var td=document.createElement("td");	
		td.innerHTML=results.data[i].compound_inchikey;		line.appendChild(td);	table.appendChild(line);
		...
		var line=document.createElement("tr"); var c=document.createElement("th"); 	c.style.textTransform= "uppercase";
		c.innerHTML="Export Options"; 	line.appendChild(c); 	var buttons=document.createElement("td");

		var div1 = document.createElement("div"); div1.setAttribute("id", "allButtons"); 	var btn = document.createElement("BUTTON");
		btn.style.marginRight = "2%"; 	btn.setAttribute("class","btn btn-secondary"); 	var t = document.createTextNode("TSV");
		
		
		btn.setAttribute("onclick", "downloadCSV('"+npa_id+"','"+results.data[i].compound_inchikey+"','"+results.data[i].origin_type+"','"+results.data[i].genus+"','"+results.data[i].compound_names+"','"+results.data[i].compound_molecular_weight+"','"+results.data[i].compound_molecular_formula+"','"+results.data[i].compound_inchi+"','"+results.data[i].compound_cluster_id+"','"+results.data[i].compound_accurate_mass+"','"+results.data[i].cd_molweight+"','"+results.data[i].cd_id+"','"+results.data[i].compound_smiles+"')");
		
		btn.appendChild(t);
		div1.appendChild(btn);		
		
		
		var btn = document.createElement("BUTTON");			btn.style.marginRight = "2%";			btn.setAttribute("class","btn btn-secondary");
		var t = document.createTextNode("MOL");			
		btn.setAttribute("onclick", "downloadMol('"+results.data[i].compound_inchi+"','"+npa_id+"')");			
		btn.appendChild(t);			div1.appendChild(btn);

		var btn = document.createElement("BUTTON");			btn.style.marginRight = "2%";
		btn.setAttribute("class","btn btn-secondary");			var t = document.createTextNode("PNG");	
		var imgData = "/custom/explore/compounds/png/"+ npa_id+"_hw500.png";
		btn.setAttribute("onclick", "downloadStructurePNG('"+npa_id+"','"+imgData+"')");
	
		btn.appendChild(t);			div1.appendChild(btn);			
		buttons.appendChild(div1);		line.appendChild(buttons);			table.appendChild(line);
		
		var line=document.createElement("tr");			var c=document.createElement("th");
		c.style.textTransform= "uppercase";			c.innerHTML="Project Molecule to Global View";
		line.appendChild(c);			var buttons=document.createElement("td");
		...
		btn.setAttribute("onclick", "window.open('/joomla/index.php/explore/global#nodeid=" + results.data[i]["compound_node_id"] + "','_self')");
...
function paginate_result(results,smiles,page,nb_pages,limit,pagi_id1,pagi_id2){
	if(results == 400){
		...
		if (total==0){
		  ...
		  create_pagination(pagi_id2,smiles,page,nb_pages,limit);
			display_paginated_result(results,smiles,page,nb_pages,limit,pagi_id1,pagi_id2)
...
function display_paginated_result(results,smiles,page,nb_pages,limit,pagi_id1,pagi_id2)
{
	var total=results.total;
...
function slide_on_pagination(pagi_id,page,nb_pages,total,limit){
	...
	if(nb_pages<=5)
	{
		...
		{
			document.getElementById(pagi_id+'_page'+(i.toString()).trim()).removeAttribute("class");
			document.getElementById(pagi_id+'_page'+(i.toString()).trim()).style.display="inline-block";
		}
	...
	else
	{
		...
		{
			if(document.contains(document.getElementById(pagi_id+'_page'+(i.toString()).trim())))
				document.getElementById(pagi_id+'_page'+(i.toString()).trim()).style.display="none";
		}
		for (i=1;i<=nb_pages;i++)
		{
			if(document.contains(document.getElementById(pagi_id+'_page'+(i.toString()).trim())))
		...
		document.getElementById(pagi_id+'_page'+page.toString().trim()).style.display="inline-block";
		document.getElementById(pagi_id+'_page'+page.toString().trim()).innerHTML=page.toString().trim();
		document.getElementById(pagi_id+'_page'+page.toString().trim()).setAttribute("class","active");
		document.getElementById(pagi_id+'_page'+nb_pages.toString().trim()).style.display="inline-block";
		if(page>3)
		{
			document.getElementById(pagi_id+'_page'+(parseInt(page)-1).toString().trim()).style.display="inline-block";
			document.getElementById(pagi_id+'_page'+(parseInt(page)-1).toString().trim()).title="«";
			document.getElementById(pagi_id+'_page'+(parseInt(page)-1).toString().trim()).innerHTML="«";
		}
		if(page<nb_pages-2)
		{
			document.getElementById(pagi_id+'_page'+(parseInt(page)+1).toString().trim()).style.display="inline-block";
		...
		if (page<=3)
		{
			document.getElementById(pagi_id+'_page2').style.display="inline-block";
			document.getElementById(pagi_id+'_page2').title="2";
			document.getElementById(pagi_id+'_page2').innerHTML="2";
			document.getElementById(pagi_id+'_page3').style.display="inline-block";
			document.getElementById(pagi_id+'_page3').title="3";
			document.getElementById(pagi_id+'_page3').innerHTML="3";
			document.getElementById(pagi_id+'_page4').style.display="inline-block";
			document.getElementById(pagi_id+'_page4').title="»";
			document.getElementById(pagi_id+'_page4').innerHTML="»";
		}
		if(page>=nb_pages-2)
		{
			document.getElementById(pagi_id+'_page'+(parseInt(nb_pages)-3).toString().trim()).style.display="inline-block";
			document.getElementById(pagi_id+'_page'+(parseInt(nb_pages)-3).toString().trim()).title="«";
			document.getElementById(pagi_id+'_page'+(parseInt(nb_pages)-3).toString().trim()).innerHTML="«";
			document.getElementById(pagi_id+'_page'+(parseInt(nb_pages)-2).toString().trim()).style.display="inline-block";
			document.getElementById(pagi_id+'_page'+(parseInt(nb_pages)-2).toString().trim()).title=(parseInt(nb_pages)-2);
			document.getElementById(pagi_id+'_page'+(parseInt(nb_pages)-2).toString().trim()).innerHTML=(parseInt(nb_pages)-2);
			document.getElementById(pagi_id+'_page'+(parseInt(nb_pages)-1).toString().trim()).style.display="inline-block";
			document.getElementById(pagi_id+'_page'+(parseInt(nb_pages)-1).toString().trim()).title=(parseInt(nb_pages)-1);
			document.getElementById(pagi_id+'_page'+(parseInt(nb_pages)-1).toString().trim()).innerHTML=(parseInt(nb_pages)-1);

		}
	}

	document.getElementById(pagi_id+'_page'+(parseInt(page)).toString().trim()).setAttribute("class","active");



	var count="Compound(s) "+((parseInt(page)-1)*limit+1).toString().trim()+" - "+(Math.min(total,parseInt(page)*limit)).toString().trim()+" on "+total.toString().trim();
...
function create_pagination(pagi_id,smiles,page,nb_pages,limit)
{
	...
	if(pagi_id == "pagination_above"){
		...
		for (i=1;i<=nb_pages;i++){
			...
			a.title = i.toString();
			a.href = "javascript:getResult('"+smiles+"',"+i.toString()+",1,"+limit.toString()+",'pagination_above','pagination_under',display_paginated_result, false, true,'', '#loadingIcon')";
		...
		//	a1.style.display="none";
			...
			*/
			...
			a.title = i.toString();
			a.href = "javascript:getResult('"+smiles+"',"+i.toString()+",1,"+limit.toString()+",'pagination_above','pagination_under',display_paginated_result, false, true,'', '#loadingIcon')";
			a.setAttribute("onclick","window.scroll({top: 1000,behavior: 'smooth'});");
	...
	,"open" : function openEditor() {
      if(currentPict != null) {
		...
		{	
			...
			image.setAttribute("src","/custom/explore/compounds/png/"+ cp.alt +"_hw500.png");
...
function allResultsGlobalView(){
	...
	getResult(globalSmiles,1,1,basicSearchTotalResults,"pagination_above","pagination_under",showGlobalCB, false, true, "","#loadingIcon" );
}
var nodeIDArr = [];
function showGlobalCB(response){
	// get all the node IDs of all the compounds and form the array
	if(response.data.length > 0){
		jQuery("#loadingIcon").show();
		nodeIDArr = []; var promiseArr = []; var npaidArr = [];
		for(var i=0; i< response.data.length; i++){		
			npaidArr.push(response.data[i].compound_id);		
		}
		var compIDnodeIDProm = compoundID_NodeID_All_query(npaidArr);
		compIDnodeIDProm.then(function(values){
			jQuery("#loadingIcon").hide();
			console.log(values);
			jQuery("#loadingIcon").hide();
			sessionStorage.setItem("basicSearchResults", JSON.stringify(values));
...
function addRow() {
	...
	var content = '<select id="customFormulaElement' + maxID + '" name="customFormulaElement' + maxID + '" style="margin: 0; margin-right: 2%; margin-bottom: 10px; width: 25%; background-color:#D9D9D9; height:35px;"><option selected value=""></option><option value="C">C</option><option value="N">N</option><option value="H">H</option><option value="O">O</option><option value="CL">Cl</option></select>';
	
	content += '<select id="customFormulaOperator' + maxID + '" name="customFormulaOperator' + maxID + '" style=" margin: 0; margin-bottom: 10px; margin-right: 2%;width: 21%; background-color:#D9D9D9; height:35px;"><option selected value=""></option><option value="lessThan"><</option><option value="greaterThan">></option><option value="equalTo">=</option></select>';

	content += '<input id="customFormulaValue' + maxID +'" name="customFormulaValue' + maxID +'" type="number" step="1" size="4" style="width:25%;margin-bottom: 10px; margin-right: 5px;" autocomplete=off><span id="btA'+maxID+'"class="icon-plus" style="cursor: pointer; margin-top: 10px; margin-left: 4px; margin-right: 10px;" onclick="addRow(this)" ></span><span id="btD' + maxID + '"class="icon-remove" onclick="remRow(this)" style="cursor: pointer;" style="margin-top: 10px;">';
